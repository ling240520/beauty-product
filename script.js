...
let activeBenefit='全部',editingId=null,currentPhoto='',openedOnly=false;
...
const withinSixMonths=date=>{const today=new Date();today.setHours(0,0,0,0);const limit=new Date(today);limit.setMonth(limit.getMonth()+6);const expiry=new Date(date+'T00:00:00');return expiry>=today&&expiry<=limit};
...
function render(){
 const q=$('#search').value.trim().toLowerCase(),brand=$('#brand-filter').value||'全部品牌',category=$('#category-filter').value||'全部類別',date=$('#date-filter').value;
 const list=products.filter(p=>{const d=daysUntil(p.expiryDate);return(!q||[p.name,p.brand,p.category,p.location||'',p.notes,...(p.benefits||[])].join(' ').toLowerCase().includes(q))&&(brand==='全部品牌'||p.brand===brand)&&(category==='全部類別'||p.category===category)&&(!openedOnly||p.opened===true)&&(activeBenefit==='全部'||(p.benefits||[]).includes(activeBenefit))&&(date==='全部日期'||date==='30日內'&&d>=0&&d<=30||date==='90日內'&&d>=0&&d<=90||date==='半年內'&&withinSixMonths(p.expiryDate)||date==='已過期'&&d<0)});
 grid.innerHTML=list.map(p=>{const [label,tone]=meta(p.expiryDate);const visual=p.photo?`<img src="${p.photo}" alt="${escapeHtml(p.name)}的產品照片">`:`<div class="bottle"><span>${escapeHtml(p.brand.slice(0,7).toUpperCase())}</span></div>`;return `<article class="product-card"><div class="product-visual" style="background:${p.color}">${visual}<span class="category">${escapeHtml(p.category)}</span><div class="card-actions"><button data-edit="${p.id}" aria-label="編輯">✎</button></div></div><div class="product-info"><p class="product-brand">${escapeHtml(p.brand)}</p><h3>${escapeHtml(p.name)}</h3><div class="tags">${(p.benefits||[]).map(b=>`<span>${escapeHtml(b)}</span>`).join('')}</div>${p.location?`<div class="location">⌖ ${escapeHtml(p.location)}</div>`:''}<div class="expiry ${tone}"><span>▣ 有效日期 ${p.expiryDate.replaceAll('-','.')}</span><strong>${label}</strong></div><div class="item-footer"><div class="opened">${p.opened?'✓ 已開封':'未開封'}</div><button class="delete-item" data-delete="${p.id}" aria-label="刪除 ${escapeHtml(p.name)}">⌫ 刪除</button></div></div></article>`}).join('');
 $('#result-count').textContent=list.length; $('#empty').hidden=!!list.length; grid.hidden=!list.length;
 const soon=products.filter(p=>withinSixMonths(p.expiryDate)).length,expired=products.filter(p=>daysUntil(p.expiryDate)<0).length; $('#total-count').textContent=products.length;$('#opened-count').textContent=products.filter(p=>p.opened).length;$('#expiry-count').textContent=soon;$('#expired-count').textContent=expired;$('#bell-count').textContent=soon;$('#expiry-stat').classList.toggle('warn',soon>0);$('#expired-stat').classList.toggle('has-expired',expired>0);
 const filtered=q||brand!=='全部品牌'||category!=='全部類別'||openedOnly||date!=='全部日期'||activeBenefit!=='全部';$('#clear-filters').hidden=!filtered;
}
function brands(){const current=$('#brand-filter').value;const list=['全部品牌',...new Set(products.map(p=>p.brand).sort())];$('#brand-filter').innerHTML=list.map(b=>`<option>${escapeHtml(b)}</option>`).join('');if(list.includes(current))$('#brand-filter').value=current;const defaults=['護膚','彩妝','精華','面霜','防曬','底妝'];const categoryList=[...new Set([...defaults,...products.map(p=>p.category).filter(Boolean)])];$('#category-options').innerHTML=categoryList.map(c=>`<option value="${escapeHtml(c)}"></option>`).join('');const selected=$('#category-filter').value;const usedCategories=[...new Set(products.map(p=>p.category).filter(Boolean).sort())];const filters=['全部類別',...usedCategories];$('#category-filter').innerHTML=filters.map(c=>`<option>${escapeHtml(c)}</option>`).join('');if(filters.includes(selected))$('#category-filter').value=selected}
...
['search','brand-filter','category-filter','date-filter'].forEach(id=>$('#'+id).addEventListener(id==='search'?'input':'change',render));
function showAllProducts(){$('#search').value='';$('#brand-filter').value='全部品牌';$('#category-filter').value='全部類別';$('#date-filter').value='全部日期';activeBenefit='全部';openedOnly=false;$('#all-nav').classList.add('active');$('#opened-nav').classList.remove('active');renderBenefits();render()}
$('#clear-filters').onclick=showAllProducts;
$('#all-nav').onclick=showAllProducts;
$('#opened-nav').onclick=()=>{$('#search').value='';$('#brand-filter').value='全部品牌';$('#category-filter').value='全部類別';$('#date-filter').value='全部日期';activeBenefit='全部';openedOnly=true;$('#opened-nav').classList.add('active');$('#all-nav').classList.remove('active');renderBenefits();render()};
$('#refilter').onclick=render;
