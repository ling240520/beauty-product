...
function brands(){const current=$('#brand-filter').value;const list=['全部品牌',...new Set(products.map(p=>p.brand).sort())];$('#brand-filter').innerHTML=list.map(b=>`<option>${escapeHtml(b)}</option>`).join('');if(list.includes(current))$('#brand-filter').value=current;const defaults=['護膚','彩妝','精華','面霜','防曬','底妝'];const categoryList=[...new Set([...defaults,...products.map(p=>p.category).filter(Boolean)])];$('#category-options').innerHTML=categoryList.map(c=>`<option value="${escapeHtml(c)}"></option>`).join('')}
